...
class AliAnalysisTaskAntipdh;
using namespace std;

ClassImp(AliAnalysisTaskAntipdh)

//________________________________________________________________________
AliAnalysisTaskAntipdh::AliAnalysisTaskAntipdh()
: AliAnalysisTaskSE()
,fVEvent(0)
,fAODEvent(0)
,fOutputList(0)
...
//________________________________________________________________________
AliAnalysisTaskAntipdh::AliAnalysisTaskAntipdh(const char* name)
...
//________________________________________________________________________
void AliAnalysisTaskAntipdh::UserCreateOutputObjects()
{
...
//________________________________________________________________________
void AliAnalysisTaskAntipdh::UserExec(Option_t *)
{
    // Main loop
...
//    }
    ...
    for (Int_t iTrack = 0; iTrack < fAODEvent->GetNumberOfTracks(); iTrack++){
		...
		Bool_t isHelium3Selected = (TMath::Abs(nSigmaTPCHe3) < 3.0); //Helium 3 is triggered in TPC only with a relaxed condition of 5, EDIT: cut reduced to 3 sigma due to large includion of background in the 5 sigma cut. See 16qt set from 260919.
...
//________________________________________________________________________
void AliAnalysisTaskAntipdh::Terminate(Option_t *)
{
    // terminate
...
//________________________________________________________________________
void AliAnalysisTaskAntipdh::CreateHistosTrack(vector<TH1*> &histos)
{
...
//________________________________________________________________________
void AliAnalysisTaskAntipdh::FillHistosTrack(vector<TH1*> &histos, AliAODTrack *track)
{
    Double_t trackP   = track->P();
...
//________________________________________________________________________
Float_t AliAnalysisTaskAntipdh::GetTOFBeta(AliAODTrack *track)
{
    float beta = -999;
...
//________________________________________________________________________
Float_t AliAnalysisTaskAntipdh::GetMass2TOF(Float_t beta, AliAODTrack *track)
{
    Float_t p = track->P();
    Float_t mass2sq = -999;
    if(!(beta==0)){
